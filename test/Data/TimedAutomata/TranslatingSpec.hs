...
spec = do
  describe "translateState" $ do
    it "non-accepting state" $ do
      let input = 
              Dot.DotNode "q0" [UnknownAttribute "init" "true", UnknownAttribute "match" "false"] in 
        let expected =
              Dot.DotNode "q0" [Shape Circle] in
          translateState input `shouldBe` expected
    it "accepting state" $ do
      let input = 
              Dot.DotNode "q0" [UnknownAttribute "init" "false", UnknownAttribute "match" "true"] in 
        let expected =
  ...
  describe "translateTransition" $ do
    it "no guard transition" $ do
      let input = 
              Dot.DotEdge "q0" "q1" [Label (StrLabel "a"), UnknownAttribute "reset" "{x0 := 0}"] in 
        let expected =
    ...
    it "guard reset transition" $ do
      let input = 
              Dot.DotEdge "q0" "q1" [Label (StrLabel "a"), UnknownAttribute "guard" "{x0 >= 10}", UnknownAttribute "reset" "{x0 := 0}"] in 
        let expected =
              Dot.DotEdge "q0" "q1" [Label (StrLabel "a, x0 >= 10 / x0 := 0")] in
          translateTransition input `shouldBe` expected
    it "multiple guard reset transition" $ do
      let input = 
              Dot.DotEdge "q0" "q1" [Label (StrLabel "a"), UnknownAttribute "guard" "{x0 >= 10, x1 < 5}", UnknownAttribute "reset" "{x0 := 0,x1 := 0}"] in 
        let expected =
