...
translateState (Dot.DotNode nodeID attributes) =
  ...
  where
    shape = case find (sameAttribute (UnknownAttribute "match" "true")) attributes of
      Nothing -> Circle
      Just (UnknownAttribute "match" "false") -> Circle
...
translateTimedAutomaton (Dot.DotGraph False True graphID (Dot.DotStmts _ _ nodes edges)) =
  ...
  where
    ...
    initialStates = filter (\ (Dot.DotNode _ attributes) -> 
                              isJust $ find (== UnknownAttribute "init" "true") attributes) nodes
